...
void gp2::Get_config(gp2::Conf_param const & param,gp2::Data & gc)
{
    ...
    if(!If || If.bad() || If.fail())
        throw gp2::Erreur(1,"probleme de lecture de la config gp2: "+gp2::Conf_param_to_str(param),gp2::Erreur::level::ERROR);

    gp2::Data dvc;
    gp2::Auto_detect(dvc);

    if(dvc.size()==0)
        throw gp2::Erreur(2,"lecture config impossible aucun materiel detecté",gp2::Erreur::level::ERROR);
...
void gp2::Set_config(gp2::Conf_param const & param,std::string value,bool debug_mode=false)
{
    ...
    if(dvc.size()==0)
        throw gp2::Erreur(3,"config impossible aucun materiel detecté",gp2::Erreur::level::ERROR);
...
void gp2::Auto_detect(gp2::Data & device)
{
    ...
    if(!If || If.bad() || If.fail())
        throw gp2::Erreur(4,"probleme de lecture des apareils connecte",gp2::Erreur::level::ERROR);
...
void gp2::Delete_file(std::string const & file,bool debug_mode=false)
{
    ...
    if(dvc.size()==0)
        throw gp2::Erreur(5,"config impossible aucun materiel detecté",gp2::Erreur::level::ERROR);
...
void gp2::List_files(gp2::Folder_data & F_data,bool debug_mode=false)
{
    ...
    if(dvc.size()==0)
        throw gp2::Erreur(6,"config impossible aucun materiel detecté",gp2::Erreur::level::ERROR);
    ...
    if(!If || If.bad() || If.fail())
        throw gp2::Erreur(7,"probleme de lecture fichier du materiel",gp2::Erreur::level::ERROR);
