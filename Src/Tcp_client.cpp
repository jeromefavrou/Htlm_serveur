...
void CSocketTCPClient::NewSocket(unsigned int const &idx)
{
    ...
    if(*Sk_Channel[idx]==INVALID_SOCKET)
        throw CSocketTCPClient::Erreur(1,"Le socket n'a pas pu etre ouvert",CSocketTCPClient::Erreur::level::WARNING);
...
void CSocketTCPClient::Connect(unsigned int const & idx,struct t_connect const & tc,typeip const & tip)
{
    ...
    if(it==Sk_Channel.end())
        throw CSocketTCPClient::Erreur(1,"ce socket ne peut etre utilisé",CSocketTCPClient::Erreur::level::ERROR);
    ...
    else if(tip==this->HOSTNAME)
    {
        ...
        if (hostinfo == nullptr)
            throw CSocketTCPClient::Erreur(1,"nom d'hote introuvable",CSocketTCPClient::Erreur::level::ERROR);
    ...
    if (connect(*Sk_Channel[idx] , (struct sockaddr *)&ServerAdress , sizeof(ServerAdress)) == SOCKET_ERROR)
        throw CSocketTCPClient::Erreur(2,"echec de connection au serveur",CSocketTCPClient::Erreur::level::ERROR);
    if(this->debug_mode)
